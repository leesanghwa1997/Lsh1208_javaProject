...
public class PokeInfoPopup extends JFrame {
    ...
    public PokeInfoPopup(PokeInfo pokeInfo) {
        ...
        // Add space between name and types
        mainPanel.add(Box.createRigidArea(new Dimension(0, 10))); // Space of 10 pixels
        ...
        // Add space between types and rank
        mainPanel.add(Box.createRigidArea(new Dimension(0, 10))); // Space of 10 pixels

        // Rank
        JLabel rankLabel = new JLabel(pokeInfo.getRank());
        rankLabel.setOpaque(false); // Make background transparent
        rankLabel.setForeground(rankColors.getOrDefault(pokeInfo.getRank(), Color.BLACK)); // Set rank text color
        rankLabel.setFont(rankFont); // Use increased font size for rank
    ...
    private void addTypeLabel(JPanel parent, String type, Font font) {
        ...
        typeLabel.setBorder(new RoundedBorder(color, 20)); // Rounded border
